...
**最要紧的那处是资源表。** 换一个 kind、换一组 namespace，表会先被 `reset` 清空，
然后等第一批 watch 数据。这中间它是**一张空表** —— 和"这个 kind 一个对象都没有"
长得一模一样。集群快的时候是一闪而过，慢的时候（远端集群、上千个对象）就是一段
看不出在不在干活的空白。

`TableDelegate` 本来就有 `loading()` 这个钩子，返回 true 时组件自己画骨架屏，
所以这里只是把状态接上：`reset()` 置位（每次 reset 后面一定跟着一次新订阅），
**任意一个 namespace 的第一批数据**到达就清除 —— 不等全部到齐，已经有的行摆出来
比藏在占位符后面有用。

**一个不接就会一直转的坑**：watch 起不来（比如某个 namespace 没权限）时，
第一批数据永远不会来，骨架屏就会转到天荒地老。所以健康状态转为 `Degraded` 时
也清除它 —— 那正是"这条 watch 坏了"从另一条路送达的消息：停止等待、露出空表，
让状态栏去说为什么。

顺带把计数也改了：骨架屏在的时候不显示数字。那时候的 0 不是真的。

**其余几处等待补上了 spinner**，让"在加载"在整个应用里长得一样：详情面板里
tone 为 `Progressing` 的 notice（取对象、找 events）、Helm 列表的"Reading releases…"、
以及启动时那句"Connecting to …" —— 连远端 API server 要好几秒，一行静止的文字
恰恰说不出它是还在试还是已经悄悄放弃了。
